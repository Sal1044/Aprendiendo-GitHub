...
public class ProyectoAula {
    ...
    public static boolean loginAdmin() {
        ...
        if (UsuarioAdmin.equals(u) && ContraseñaAdmin.equals(c))
            return true;
    ...
    public static boolean loginContabilidad() {
        ...
        System.out.print("Contraseña: ");
        String c = entrada.nextLine();
        if (UsuarioConta.equals(u) && ContraseñaConta.equals(c))
            return true;
        System.out.println("\nCredenciales inválidas.");
        return false;
    }
    ...
    // INGRESO CLIENTE: tipo, discapacitado, placa (si aplica), asignación
    ...
    // Colocar en una fila especifica (busca primer libre en esa fila) y devuelve la
    // posición [i,j] o null
    ...
    // Colocar en varias filas dada una lista de indices y devuelve la posición
    // [i,j] o null
    ...
    public static void pagoCliente() {
        ...
        if (valorCompra >= 100000)
            minutosGratisPorCompra = 180; // 3 horas gratis
        else if (valorCompra > 50000)
            descuento = 0.25; // 25% descuento
        ...
        if (minutosGratisPorCompra > 0) {
            if (minutosACobrar <= minutosGratisPorCompra)
                minutosACobrar = 0;
            else
                minutosACobrar -= minutosGratisPorCompra;
        }
    ...
    public static void salidaCliente() {
        ...
        if (tipo == 3) {
            ...
            parqueadero[ib][jb] = "LIBRE";
            horaEntrada[ib][jb] = null;
            tipoEnPuesto[ib][jb] = "";
            pagado[ib][jb] = false;
    ...
    // Buscar primera bicicleta (fila 2 -> indice 1) y devolver su posición [i,j] o
    // null
    ...
    // Ingreso manual administrador: ahora el usuario verá filas 1..6 y columnas
    // 1..5
    public static void ingresoManualAdmin() {
        ...
        if (placa.isEmpty())
            placa = tipoStr.equals("BICI") ? placa : "SIN_PLACA";
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 5; j++) {
                ...
                if (placa.equals(parqueadero[i][j])) {
                    ...
                    System.out.println(
                            "║ Entrada:      " + horaEntrada[i][j].format(DateTimeFormatter.ofPattern("HH:mm")));
    ...
    public static double leerDouble() {
        try {
            ...
            System.out.println("\nError: valor inválido, se toma 0.");
            return 0;
        }
    }
